...
int main(int argc, char* argv[]) {
	...
	printf("deviceCountInNode: %d\n", deviceCountInNode);
    // Alloc data on CPU
    Complex *data_cpu[deviceCountInNode], *data_cpu_out[deviceCountInNode], *node_data_dev[deviceCountInNode], *node_data_dev_back[deviceCountInNode];
    ...
    #pragma omp parallel for num_threads(deviceCountInNode)
    {
        for (int i = 0; i < deviceCountInNode; ++i) {
            ...
            //ROCM_CHECK(hipSetDevice(globalIdx));
            ...
            for (int j = 0; j < dataCountInNode[i]; ++j) {
                ...
                // data_cpu[i][j][0] = data_cpu[i][j][1] = j;
            }
            ...
            fft_mpi_3d_plan_p planBack = fft_mpi_plan_dft_c2c_3d(N[0], N[1], N[2], outDev, inDev, node_data_dev_back, MPI_COMM_WORLD, i, deviceCountInNode, totalDeviceCount, BACKWARD);
            ...
            double maxErr = -1.0;
            for (int j = 0; j < dataCountInNode[i]; ++j) {
                double tmp1 = data_cpu[i][j][0] - data_cpu_out[i][j][0], tmp2 = data_cpu[i][j][1] - data_cpu_out[i][j][1],
    ...
    if (mpi_rank == 0) {
        ...
        // printf("Forward FFT time: %lf\nGflops: %lf\nMax error: %.5e\n", forwardTimeTotal, gflops, maxErrTotal);
        std::cout << "\n----------------------------------------------------------------------------- \n";
