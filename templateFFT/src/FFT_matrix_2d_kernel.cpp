...
__device__ inline void complex_mul(wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> &frag_F_real, wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> &frag_F_imag,
                                   wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> &frag_in_real, wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> &frag_in_imag,
                                   wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> &frag_out_real, wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> &frag_out_imag)
{
    half initNum = 0.0;
    wmma::fill_fragment(frag_out_real, initNum);
    wmma::fill_fragment(frag_out_imag, initNum);
...
__device__ inline void complex_mul_acc(wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> &frag_F_real, wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> &frag_F_imag,
                                       wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> &frag_in_real, wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> &frag_in_imag,
                                       wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> &frag_out_real, wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> &frag_out_imag)
{
    half initNum = 0.0;
    wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_buf_real;
    wmma::fill_fragment(frag_buf_real, initNum);
...
{
    float2 tmpa = __half22float2(a);
    float2 tmpb = __half22float2(b);

    return {tmpa.x * tmpb.x - tmpa.y * tmpb.y, tmpa.x * tmpb.y + tmpa.y * tmpb.x};
...
__device__ inline half2 const cmul_mixed(const half2 &a, const float2 &b)
{
    float2 tmpa =  __half22float2(a);
    //float2 tmpb =  __half22float2(b);
    return {tmpa.x * b.x - tmpa.y * b.y, tmpa.x * b.y + tmpa.y * b.x};
}
...
__global__ void layer_256_0(half2 *in, half *F_real, half *F_imag)
{
    ...
    wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> frag_F_real;
    wmma::fragment<matrix_a, WMMA_M, WMMA_N, WMMA_K, half, row_major> frag_F_imag;
    ...
    {
        wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_real;
        wmma::fragment<accumulator, WMMA_M, WMMA_N, WMMA_K, half> frag_out_imag;
        wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> frag_in_real;
        wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> frag_in_imag;
        wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> frag_in_tmp0;
        wmma::fragment<matrix_b, WMMA_M, WMMA_N, WMMA_K, half, col_major> frag_in_tmp1;
    ...
    */
        ...
        for (int j = 0; j < 16; ++j)
        {
            ...
            frag_in_real.x[j] = __half22float2(in_ele).x;
            frag_in_imag.x[j] = __half22float2(in_ele).y;
        ...
        for (int j = 0; j < 8; ++j)
        {
            int row = raw_row;
            int col = j + raw_col;
            half2 frag_out = {frag_out_real.x[j], frag_out_imag.x[j]};
            smem_in[warp_start + row * 16 + col] = frag_out;
        }
    }
    ...
    */
    ...
    for (int i = 0; i < 256 * CONT_SIZE; i += NUM_WARP * 16 * 16)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            frag_in_real.x[8 + j] = frag_in_real.x[j] = __half22float2(ele).x;
            frag_in_imag.x[8 + j] = frag_in_imag.x[j] = __half22float2(ele).y;
        }
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            frag_in_real.x[8 + j] = frag_in_real.x[j] = __half22float2(in_ele).x;
            frag_in_imag.x[8 + j] = frag_in_imag.x[j] = __half22float2(in_ele).y;
        }
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            half2 frag_out = {frag_out_real.x[j], frag_out_imag.x[j]};
            in[block_start + warp_start + row * 16 + col] = frag_out;
    ...
    */
    ...
    for (int i_start = 0; i_start < 256 * CONT_SIZE; i_start += NUM_WARP * 256)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            ...
            frag_in_real.x[j] = __half22float2(ele).x;
            frag_in_imag.x[j] = __half22float2(ele).y;
            // twiddle_factor = cmul(twiddle_factor, twiddle_unit);
        }
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            int col = j + acc_col;
            half2 frag_out = {frag_out_real.x[j], frag_out_imag.x[j]};
            smem_in[warp_start + row * 32 + col] = frag_out;
        }
    }
    ...
    */
    ...
    for (int i_start = 0; i_start < CONT_SIZE / NUM_WARP; i_start++)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            int col = j + acc_col;
            half2 frag_out = {frag_out_real.x[j], frag_out_imag.x[j]};
            smem_in[warp_start + row * 512 + col] = frag_out;
        }
    }
...
__global__ void layer_512_1(int step, half2 *in, half *F_real, half *F_imag)
{
    ...
    for (int i_start = 0; i_start < 512 * CONT_SIZE; i_start += NUM_WARP * 256)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            int col = j + acc_col;
            half2 frag_out ={frag_out_real.x[j], frag_out_imag.x[j]};
            smem_in[warp_start + row * CONT_SIZE + col] = frag_out;
        }
    }
    ...
    for (int i_start = 0; i_start < 4; i_start++)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            ...
            int col = j + acc_col;
            half2 frag_out = {frag_out_real.x[j], frag_out_imag.x[j]};
            smem_in[warp_start + row * 256 + col] = frag_out;
        }
    }
...
__global__ void layer_1024_0(half2 *in, half *F_real, half *F_imag)
{
    ...
    for (int i = 0; i < 1024 * CONT_SIZE; i += NUM_WARP * 32 * 4)
    {
        ...
        in[block_start + eid + 256] = ele0 + half2({__half22float2(ele1).y, -__half22float2(ele1).x}) - ele2 + half2({-__half22float2(ele3).y, __half22float2(ele3).x});
        in[block_start + eid + 512] = ele0 - ele1 + ele2 - ele3;
        in[block_start + eid + 768] = ele0 + half2({-__half22float2(ele1).y, __half22float2(ele1).x}) - ele2 + half2({__half22float2(ele3).y, -__half22float2(ele3).x});
    }
}
...
__global__ void layer_1024_0_A100(half2 *in, half *F_real, half *F_imag)
{
    ...
    for (int i = 0; i < 1024 * CONT_SIZE; i += NUM_WARP * 16 * 16)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            int row = raw_row + j % 4 / 2 * 8 + j % 2;
            int col = raw_col + j / 4 * 8;
            half2 in_ele = {frag_in_real[j], frag_in_imag[j]};
            in_ele = cmul(in_ele, W_N_K(256, row * col));
            frag_in_real.x[8 + j] = frag_in_real.x[j] = __half22float2(in_ele).x;
            frag_in_imag.x[8 + j] = frag_in_imag.x[j] = __half22float2(in_ele).y;
        }
...
void FFTExec(FFTMatrixHandle plan, half *data)
{
    ...
    //printf("%pf", plan.layer_0[plan.mergings[0]]);

    //printf("gridSize = %d;  blockSize = %d;   sharedMemSize = %lu\n", plan.Nx * plan.Ny * plan.N_batch / n_cont[plan.mergings[0]] / RADIX, threads.x*threads.y*threads.z, RADIX * sizeof(half2) * n_cont[plan.mergings[0]]);

    hipLaunchKernelGGL(plan.layer_0[plan.mergings[0]],
                       plan.Nx * plan.Ny * plan.N_batch / n_cont[plan.mergings[0]] / RADIX,
                       threads,
                       RADIX * sizeof(half2) * n_cont[plan.mergings[0]],
                       0, 
                       (half2 *)data, plan.F_real, plan.F_imag);
    step *= RADIX;

    RADIX = plan.Nx;
    blocks = {static_cast<uint32_t>(step / n_cont[plan.mergings[1]]), static_cast<uint32_t>(plan.N_batch * plan.Nx * plan.Ny / step / RADIX)};
    ...
    //std::cout<<plan.mergings[1]<<std::endl;
    hipLaunchKernelGGL(plan.layer_1[plan.mergings[1]],
                       blocks,
                       threads,
                       RADIX * sizeof(half2) * n_cont[plan.mergings[1]],
                       0,
                       step, (half2 *)data, plan.F_real, plan.F_imag);
    step *= RADIX;
}
...
//#pragma omp parallel for
    ...
    ErrChk(hipMalloc(&plan->F_real, sizeof(half) * 256));
    ErrChk(hipMemcpy(plan->F_real, plan->F_real_tmp, sizeof(half) * 256, hipMemcpyHostToDevice));
    ErrChk(hipMalloc(&plan->F_imag, sizeof(half) * 256));
    ErrChk(hipMemcpy(plan->F_imag, plan->F_imag_tmp, sizeof(half) * 256, hipMemcpyHostToDevice));
}