...
FFTResult sample_FFT_2D_double(GPU* GPU, uint64_t file_output, FILE* output)
{
	...
	if (file_output)
		fprintf(output, "1 - FFT + iFFT C2C 2D in double precision LUT\n");
	printf("1 - FFT + iFFT C2C 2D in double precision LUT\n");
	...
	res = hipMemcpy(buffer_output, app.configuration.buffer[0], bufferSize, hipMemcpyDeviceToHost);
    if (res != hipSuccess) return FFT_ERROR_FAILED_TO_COPY;
	...
	double opscount = (double)5 * N * log(X * Y) / log(2.0);
	...
	if (printResult == 1) {
        ...
        std::cout << "element " << i << " input:  (" << buffer_input[2 * i] << ","
                  << buffer_input[2 * i + 1] << ")"
                  << " output: (" << buffer_output[2 * i]/(X * Y) << "," << buffer_output[2 * i + 1]/(X * Y) << ")"
                  << std::endl;
        }
        for (uint64_t i = input_size - 8; i < input_size; i++) {
            std::cout << "element " << i << " input:  (" << buffer_input[2 * i] << ","
                      << buffer_input[2 * i + 1] << ")" << " output: ("
                      << buffer_output[2 * i]/(X * Y) << "," << buffer_output[2 * i + 1]/(X * Y) << ")" << std::endl;
        }
    }

	double tmp1, tmp2, tmp3 ,maxErr = 0.0;
	for (int i = 0; i < N; i++) {
		tmp1 = buffer_input[2*i]-buffer_output[2*i]/(X * Y);
		tmp2 = buffer_input[2*i+1]-buffer_output[2*i+1]/(X * Y);
	...
	num_tot_transfers *= 4;
	char filename[30] = "batch_result2D.csv";
...
int main(int argc, char *argv[]) {
	...
	Z = (uint64_t)(64 * 32 * (uint64_t)pow(2, 15))/(X * Y);
    ...
    FFTResult resFFT = sample_FFT_2D_double(&GPU, file_output, output);
