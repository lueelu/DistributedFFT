...
void call_slab_local_transpose_z_to_x_uneven_optimized(double *data_in, double *data_out, int x_size, int y_size, int z_size, int y_divide, int y_last, int total_device, int direction) 
{
    ...
    // if (direction == 1) {
    //     int gridZ = ceil((double)z_size / 1024), blkX = z_size<=1024? z_size: 1024;
    //     dim3 grid_size(x_size, y_size, gridZ), block_size(blkX);
    //     hipLaunchKernelGGL(slab_local_transpose_z_to_x_uneven_forward_optimized, grid_size, block_size, 0, 0, data_in, data_out, y_divide, y_last, z_size, total_device);
    // }
    // else if (direction == -1) {
    //     dim3 grid_size(x_size, y_size, 1), block_size(z_size);
...
__global__ void slab_local_transpose_z_to_x_uneven_forward_optimized(double *data_in, double *data_out, int y_divide, int y_last, int z_last, int total_device) 
{
