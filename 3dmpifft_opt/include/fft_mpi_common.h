...
#define OPTFFT_CHECK(stmt)                                                \
do {                                                                    \
   FFTResult fft_errno = (stmt);                                           \
   if (FFT_SUCCESS != fft_errno) {                                                    \
       fprintf(stderr, "[%s:%d] Optimized FFT call '%s' failed with %d\n",  \
        __FILE__, __LINE__, #stmt, FFT_SUCCESS);    \
       exit(EXIT_FAILURE);                                              \
   }                                                                    \
   assert(FFT_SUCCESS == fft_errno);                                         \
} while (0)
