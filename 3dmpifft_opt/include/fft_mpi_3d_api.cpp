...
fft_mpi_3d_plan_p fft_mpi_plan_dft_c2c_3d(longInt64 n0, longInt64 n1, longInt64 n2, Complex *in, Complex *out, Complex **node_data, MPI_Comm comm, int devIdx, int devCountInNode, int totalDevCount, int direction) 
{
    ...
    ROCM_CHECK(hipMemcpy(plan->bufferDev1, plan->inDev, plan->maxDataCountInDevice * sizeof(Complex), hipMemcpyDeviceToDevice));

    plan->nodeDataDev = node_data;
    plan->nodeDataDev[plan->locGPUIdx] = plan->bufferDev1;

    ROCM_CHECK(hipMalloc(&plan->bufferYZ, plan->N[1] * plan->N[2] * sizeof(Complex)));
...
void fft_mpi_destroy_plan(fft_mpi_3d_plan_p plan) 
{
    ...
    ROCM_CHECK(hipFree(plan->bufferYZ));
...
void setFFTPlans(fft_mpi_3d_plan_p plan) 
{
    ...
    // Optimized FFT plans
    // 2d
    plan->configYZ.FFTdim = 2;
    plan->configYZ.size[0] = plan->N[2];
    plan->configYZ.size[1] = plan->N[1];
    plan->configYZ.size[2] = 1;
    plan->configYZ.doublePrecision = true;
    plan->configYZ.useLUT = true;
    plan->configYZ.device = &plan->gpuInfoYZ.device;
    uint64_t bufferSizeYZ = plan->N[1] * plan->N[2] * sizeof(Complex);
    plan->configYZ.bufferSize = &bufferSizeYZ;
    if (plan->direction == FORWARD) {
        plan->configYZ.makeForwardPlanOnly = true;
        plan->configYZ.buffer = (void**)&plan->bufferDev1;
    }
    else if (plan->direction == BACKWARD) {
        plan->configYZ.makeInversePlanOnly = true;
        plan->configYZ.buffer = (void**)&plan->bufferDev2;
    }
    OPTFFT_CHECK(initializeFFT(&plan->appYZ, plan->configYZ));
    if (plan->direction == FORWARD)
        OPTFFT_CHECK(setFFTArgs(&plan->gpuInfoYZ, &plan->appYZ, &plan->paramsYZ, 0));
    else if (plan->direction == BACKWARD)
        OPTFFT_CHECK(setFFTArgs(&plan->gpuInfoYZ, &plan->appYZ, &plan->paramsYZ, 1));
    // 1d
    plan->configX.FFTdim = 1;
    plan->configX.size[0] = plan->N[0];
    // plan->configX.size[1] = 1;
    int timesX = plan->isLastDevice? plan->lastExchangeN1 * plan->N[2]: (ceil((double)plan->N[1] / plan->totalDevCount) * plan->N[2]);
    plan->configX.size[1] = timesX;

    plan->configX.size[2] = 1;
    plan->configX.doublePrecision = true;
    plan->configX.useLUT = true;
    plan->configX.device = &plan->gpuInfoX.device;
    // uint64_t bufferSizeX = plan->N[0] * sizeof(Complex);
    uint64_t bufferSizeX = plan->N[0] * timesX * sizeof(Complex);
    plan->configX.bufferSize = &bufferSizeX;
    if (plan->direction == FORWARD) {
        plan->configX.makeForwardPlanOnly = true;
        plan->configX.buffer = (void**)&plan->bufferDev2;
    }
    else if (plan->direction == BACKWARD) {
        plan->configX.makeInversePlanOnly = true;
        plan->configX.buffer = (void**)&plan->bufferDev1;
    }
    OPTFFT_CHECK(initializeFFT(&plan->appX, plan->configX));
    if (plan->direction == FORWARD)
        OPTFFT_CHECK(setFFTArgs(&plan->gpuInfoX, &plan->appX, &plan->paramsX, 0));
    else if (plan->direction == BACKWARD)
        OPTFFT_CHECK(setFFTArgs(&plan->gpuInfoX, &plan->appX, &plan->paramsX, 1));    
...
{ 
    ...
    if (plan->direction == FORWARD) {
        for (int i = 0; i < times; ++i) {
            Complex *targetAddr = plan->bufferDev1 + i * plan->N[1] * plan->N[2];
            // Complex *targetAddr = plan->bufferDev2 + i * plan->N[1] * plan->N[2];
            // ROCM_CHECK(hipMemcpy(plan->bufferYZ, targetAddr, plan->N[1] * plan->N[2] * sizeof(Complex), hipMemcpyDeviceToDevice));
            plan->appYZ.configuration.buffer = (void**)&(targetAddr);
            
            ROCM_CHECK(launchFFTKernel(&plan->appYZ, 0));
            // ROCM_CHECK(hipMemcpy(targetAddr, plan->bufferYZ, plan->N[1] * plan->N[2] * sizeof(Complex), hipMemcpyDeviceToDevice));

            // Complex *tmp = (Complex*)malloc(5 * sizeof(Complex));
            // ROCM_CHECK(hipMemcpyDtoH(tmp, plan->bufferDev1, 5 * sizeof(Complex)));
            // for (int j = 0; j < 5; ++j) {
            //     printf("------%lf %lf", tmp[j][0], tmp[j][1]);
            // }
            // printf("\n");
        }
    }
        ...
        for (int i = 0; i < times; ++i) {
            Complex *targetAddr = plan->bufferDev2 + i * plan->N[1] * plan->N[2];
            plan->appYZ.configuration.buffer = (void**)&targetAddr;
            ROCM_CHECK(launchFFTKernel(&plan->appYZ, 1));
        }
    }
...
void fftX(fft_mpi_3d_plan_p plan) 
{
    ...
    // if (plan->direction == FORWARD) {
    //     ROCFFT_CHECK(rocfft_execute(plan->rocPlanX, (void**)&plan->bufferDev2, (void**)&plan->bufferDev2, plan->rocPlanInfoX));
    // }
    // else if (plan->direction == BACKWARD)
    //     // ROCM_CHECK(hipMemcpy(plan->inDev, plan->inDev, plan->maxDataCountInDevice * sizeof(Complex), hipMemcpyDeviceToDevice));
    //     ROCFFT_CHECK(rocfft_execute(plan->rocPlanX, (void**)&plan->bufferDev2, (void**)&plan->bufferDev2, plan->rocPlanInfoX));

    int timesX = plan->isLastDevice? plan->lastExchangeN1 * plan->N[2]: (ceil((double)plan->N[1] / plan->totalDevCount) * plan->N[2]),
        ySize = plan->isLastDevice? plan->lastExchangeN1: ceil((double)plan->N[1] / plan->totalDevCount);
    if (plan->direction == FORWARD) {

        const int tSize[3] = { plan->N[2], ySize, plan->N[0] },
                permutation[3] = {2, 0, 1};
        int result = cut_transpose3d((data_t*)plan->bufferDev2, (data_t*)plan->bufferDev1, tSize, permutation, 1);
        if (result < 0) {
            printf("fail to do stride transpose in x dimension!\n");
            exit(EXIT_FAILURE);
        }
        // for (int i = 0; i < timesX; ++i) {
        //     Complex *targetAddr = plan->bufferDev2 + i * plan->N[0];
        //     plan->appX.configuration.buffer = (void**)&(targetAddr);
        //     ROCM_CHECK(launchFFTKernel(&plan->appX, 0));
        // }
        ROCM_CHECK(launchFFTKernel(&plan->appX, 0));
    }
    else if (plan->direction == BACKWARD) {
        // for (int i = 0; i < timesX; ++i) {
        //     Complex *targetAddr = plan->bufferDev1 + i * plan->N[0];
        //     plan->appX.configuration.buffer = (void**)&(targetAddr);
        //     ROCM_CHECK(launchFFTKernel(&plan->appX, 1));
        // }
        // Complex *targetAddr = plan->bufferDev1;
        // plan->appX.configuration.buffer = (void**)&(targetAddr);
        ROCM_CHECK(launchFFTKernel(&plan->appX, 1));
        // call_local_transpose_z_to_x_strided_backward((double*)plan->bufferDev1, (double*)plan->bufferDev2, plan->N[0], ySize, plan->N[2]);
        const int tSize[3] = { plan->N[0], ySize, plan->N[2] },
                permutation[3] = {1, 2, 0};
        int result = cut_transpose3d((data_t*)plan->bufferDev2, (data_t*)plan->bufferDev1, tSize, permutation, 1);
        if (result < 0) {
            printf("fail to do stride transpose in x dimension!\n");
...
void localTransposeUneven(fft_mpi_3d_plan_p plan) 
{
    ...
    if (plan->direction == FORWARD) {
        // if (z_size <= 1024) {
            call_slab_local_transpose_z_to_x_uneven_optimized((double*)plan->bufferDev1, (double*)plan->bufferDev2, x_size, y_size, z_size, y_divide, y_last, plan->totalDevCount, plan->direction);
        // }
        // else {
        //     call_slab_local_transpose_z_to_x_uneven((double*)plan->bufferDev1, (double*)plan->bufferDev2, x_size, y_size, z_size, y_divide, y_last, plan->totalDevCount, plan->direction);
        // }
    }
    else if (plan->direction == BACKWARD) {
        if (z_size <= 1024) {
...
void slabAlltoall(fft_mpi_3d_plan_p plan) 
{
    ...
    for (int i = 0; i < plan->devCountInNode; ++i) {
        ...
        ROCM_CHECK(hipMemcpyPeerAsync(plan->nodeDataDev[i]+recv_offset, i, plan->bufferDev2+send_offset, plan->locGPUIdx, send_bytes, plan->stream1));
        ...
        if (i != plan->mpiRank) {
            for (int j = 0; j < dev_count; ++j) {
                int dev_idx = i * normal_dev_count + j,
                    roffset = plan->tInfo.roffset[dev_idx], soffset = plan->tInfo.soffset[dev_idx],
                    rcount = plan->tInfo.rcount[dev_idx], scount = plan->tInfo.scount[dev_idx],
                    send_tag = plan->globalDevIdx * 10 + dev_idx, recv_tag = dev_idx * 10 + plan->globalDevIdx;
#pragma omp critical 
{
                // MPI_CHECK(MPI_Irecv(plan->bufferDev2+roffset, rcount*2, MPI_DOUBLE, i, recv_tag, MPI_COMM_WORLD, &recv_reqs[req_cnt]));
                MPI_CHECK(MPI_Irecv(plan->bufferDev1+roffset, rcount*2, MPI_DOUBLE, i, recv_tag, MPI_COMM_WORLD, &recv_reqs[req_cnt]));
                MPI_CHECK(MPI_Isend(plan->bufferDev2+soffset, scount*2, MPI_DOUBLE, i, send_tag, MPI_COMM_WORLD, &send_reqs[req_cnt++]));
}
                ...
                // printf("rank: %d\trecv: from %d, tag: %d\tsend: to %d, tag: %d\tsc: %d, so: %d, rc: %d, ro: %d\n", plan->mpiRank, i, recv_tag, i, send_tag, scount, soffset, rcount, roffset);
            }
        }
    }
...
//     int tmpCount = 0, flag_send[req_cnt], flag_recv[req_cnt];
//     memset(flag_send, 0, sizeof(int) * req_cnt);
//     memset(flag_recv, 0, sizeof(int) * req_cnt);
//     while (tmpCount < req_cnt * 2) {
//         for (int i = 0; i < req_cnt; ++i) {
//             if (flag_recv[i] == 1)
//                 continue;
// #pragma omp critical 
// {
//             MPI_CHECK(MPI_Test(&recv_reqs[i], &flag_recv[i], MPI_STATUS_IGNORE));
// }
//             if (flag_recv[i] == 1)
//                 ++tmpCount;
//         }
...
// #pragma omp critical 
// {
//             MPI_CHECK(MPI_Test(&send_reqs[i], &flag_send[i], MPI_STATUS_IGNORE));
// }
//             if (flag_send[i] == 1)
//                 ++tmpCount;
//         }
...
void debugLocalData(fft_mpi_3d_plan_p plan, int flag, int type)
{
    ...
    if (flag == 1) {
        ...
        // data_ptr = (Complex*)*plan->configYZ.buffer;
    }
    else {
